...
void slth::TransformComponent::MoveTowards(Vector2f target, float deltaTime, float slowRadius)
{
	...
	{
		// See documentation about slowing. This doesn't work well because of
		// the nature of my pathing algorithm. By the time it's reached the 
		// final node, it's too late to slow down.
...
void slth::TransformComponent::RotateTowards(float desiredAngle, float deltaTime)
{
	...
	desiredAngle = std::fmod(desiredAngle, 360.f);
