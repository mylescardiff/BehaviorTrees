...
void slth::GridSystem::Draw(slth::IGraphics* pGraphics)
{
	...
	for (GridTile& tile : m_tiles)
	{
		...
		if (tile.GetVisited() || tile.GetGScore() != GridTile::kInfinity || tile.GetIndex() == m_pathEndIndex)
		{
			...
			slth::Color markerColor = slth::Color::MediumGray();
			if (tile.GetIsInPath())
			{
			...
			{
				markerColor = slth::Color::DarkGray();
			}
		
			//markerColor.m_opacity = 0.5f;
	...
	for (int y = 0; y < (int)m_size.x; ++y) {
		...
		pGraphics->DrawLine(top, bottom, slth::Color::DarkGray());
	}
	for (int x = 0; x < (int)m_size.y; ++x) {
		...
		pGraphics->DrawLine(left, right, slth::Color::DarkGray());
