...
namespace slth
{
	...
	public:
		...
		/// <returns></returns>
		SLTH_API const float DistanceTo(Vector2f otherVector)
		{
		...
		SLTH_API bool IsInCone(const float radius, const float visionArc, const float rotation, Vector2f point) const
		{
			
			float angleBetween = slth::Vector2f::AngleBetween(*this, point);
			float distanceTo = point.DistanceTo(*this);
			if (std::abs(angleBetween - rotation) < visionArc / 2.f && distanceTo <= radius)
			{
				return true;
			}
			return false;
		...
		SLTH_API Vector2f Rotate(const float degrees)
		{
